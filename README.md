<h1 align="center">👋 Olá, eu sou o Alejandro</h1>

<p align="center">
  🎓 Estudante de Análise e Desenvolvimento de Sistemas (ADS) na Unopar.
💻 Desenvolvedor em formação com foco em <strong>C# e desenvolvimento backend</strong>.
</p>

---

### 🚀 Sobre mim
Minha jornada na programação começou com <strong>Java</strong>, onde tive meu primeiro contato mais aprofundado com <strong> Programação Orientada a Objetos</strong>, desenvolvimento de aplicações e boas práticas de desenvolvimento. Atualmente estou direcionando meus estudos para o ecossistema <strong> C# e .NET</strong>, buscando construir uma base sólida para atuar no desenvolvimento de software.

- 🌱 Estudando: **C# e .Net**
- 💡 Interesse em: **Desenvolvimento Backend** e **DevOps**
- 🐧 Utilizo **Mac OS X** como ambiente de desenvolvimento

---

### 🧰 Tecnologias e Ferramentas

<div align="center">

| Categoria | Ferramentas |
|------------|-------------|
| 💻 Linguagens | C#, Java, JavaScript |
| 📚 Frameworks | ASP.NET, EntityFramework |
| ⚙️ DevOps | Bash, Git, Linux |
| 🗄️ Banco de Dados | PostgreSQL, SQL Server |
| 🐧 Sistema Operacional | Mac OS X |

</div>

---

### 📚 Certificações
- DevOps e Agile Culture — FIAP (60h)  
- Linux Fundamentos — FIAP (40h)  
- Git e GitHub — Cursa (8h)  
- O Papel do DevOps na Transformação Digital — ENAP (20h)

---

### 🧩 Projetos em andamento
- **[Gerenciador de Material Java](https://github.com/alejandro-s23/SiGMa)** → CRUD simples em Java com JDBC e PostgreSQL
- **[Gerenciador de Biblioteca Autônoma](https://github.com/alejandro-s23/Biblioteca-Clandestina)** (Em desenvolvimento) → Projeto real Web MVC + SQLServer com ASP.Net e EntityFramework

---

### 📫 Onde me encontrar
<a href="https://www.linkedin.com/in/alejandro-s-santos/" target="_blank">💼 LinkedIn</a>  
📧 **alejandrosouza277353@gmail.com**  

---

⭐ _“O aprendizado contínuo é o combustível da evolução na tecnologia.”_
